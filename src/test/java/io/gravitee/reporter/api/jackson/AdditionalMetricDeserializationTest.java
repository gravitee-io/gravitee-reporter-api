...
class AdditionalMetricDeserializationTest {
    ...
    void should_deserialize_a_keyword_metric_as_a_list_when_its_value_is_an_array() throws JsonProcessingException {
        ...
        assertThat(result.get(0)).isInstanceOfSatisfying(AdditionalMetric.KeywordMetric.class, metric ->
            assertThat(metric.value()).isEqualTo("foo")
        );
        assertThat(result.get(1)).isInstanceOfSatisfying(AdditionalMetric.KeywordListMetric.class, metric ->
            assertThat(metric.value()).isEmpty()
        );
        assertThat(result.get(2)).isInstanceOfSatisfying(AdditionalMetric.KeywordListMetric.class, metric ->
            assertThat(metric.value()).containsExactly("foo", "bar")
        );
    }
    ...
    void should_ignore_non_scalar_elements_of_a_keyword_list() throws JsonProcessingException {
        ...
        assertThat(result.get(0)).isInstanceOfSatisfying(AdditionalMetric.KeywordListMetric.class, metric ->
            assertThat(metric.value()).containsExactly("1", "true", "ok")
        );
    }
    ...
    void should_fail_on_a_keyword_list_holding_a_null_value() {
        assertThatCode(() -> new AdditionalMetric.KeywordListMetric("keyword_values", Arrays.asList("foo", null))).isInstanceOf(
